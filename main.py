...
population_size = 100
chromosome_length = 16  # 8 bits para a parte inteira e 8 bits para a parte decimal
mutation_rate = 0.01
...
for generation in range(num_generations):
    ...
    evaluated_population = [(chromosome, fitness(int(chromosome[:8], 2) + int(chromosome[8:], 2)/255)) for chromosome in population]
...
print(f'Melhor solucao: f({int(best_solution[:8], 2) + int(best_solution[8:], 2)/255}) = {best_fitness}')
